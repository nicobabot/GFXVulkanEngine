...
{
    float3 lightPos = float3(-5, 0, 0);
    float3 lightDir = normalize(lightPos - input.fragPos);
    float ambientColor = 0.085;
    float diffuseColor = max(dot(input.normal, lightDir), 0);

    float shininess = 32.0f;
    float3 viewDir = normalize(input.viewPosF - input.fragPos);
    float3 halfwayDir = normalize(lightDir + viewDir);
    float specularColor = pow(max(dot(input.normal, halfwayDir), 0), shininess);

    float4 finalColor = float4((ambientColor + diffuseColor) * fragmentColor.rgb + specularColor, 1);
    return finalColor;
}
...
float4 PSMain(PSInput input) : SV_TARGET
{
    ...
    //return DirectionalLight(texColor, input);
    return texColor;
}